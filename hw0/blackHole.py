...
class BlackHole(ts.Animation):
    def on_init(self):
        self.img = ti.Vector(4, dt=ti.f32, shape=(WIDTH, HEIGHT))
        self.define_input()
    ...
    @ti.kernel
    def on_render(self, rt: float):
        for i, j in self.img:
            uv = (ti.Vector([i, j]) - 0.5 * ti.Vector([WIDTH, HEIGHT])) / HEIGHT
            ro = ti.Vector([0.0, ti.cos(rt * 0.5) * 10.0, ti.sin(rt * 0.5) * 10.0])
            rd = ti.Vector([uv[0], uv[1], 1.0]).normalized()

            # rotate
            rmat = rotate(0.5 * (rt + PI))
            new_rdx = rmat @ ti.Vector([rd[1], rd[2]])
            rd[1] = new_rdx[0]
            rd[2] = new_rdx[1]

            # color
            self.img[i, j] = raymarch(ro, rd)
...
def raymarch(ro, rd):
    ...
    for _ in range(700):
        ...
        p += rd * max(dS, 0.01)
    gcol = getGlow(glow)
    c_rgb = ts.mix(ti.Vector([0.0, 0.0, 0.0]), ti.Vector([gcol[0], gcol[1], gcol[2]]), gcol[3])
